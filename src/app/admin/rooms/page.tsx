    ...
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-foreground-muted text-sm">{rooms.length} tipe kamar tersedia</p>
            </div>
          </div>
          <button onClick={openAdd} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary to-primary-dark text-white font-medium text-sm shadow-lg shadow-primary/20 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 active:scale-95 w-fit">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {rooms.map((room, i) => (
          <div
            key={room.id}
            className={`bg-white rounded-2xl border border-border overflow-hidden group transition-all duration-300 ${
              animateOut === room.id
                ? "opacity-0 scale-95 -translate-y-2"
                : "hover:shadow-xl hover:-translate-y-1.5 hover:border-primary/15"
            }`}
            style={{
              opacity: animateOut === room.id ? 0 : 0,
              animation: animateOut === room.id ? "none" : `fade-in-up 0.4s ease-out ${i * 80}ms forwards`,
            }}
          >
            {/* IMAGE */}
            <div className="relative h-48 overflow-hidden">
              ...
              <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm text-foreground px-3 py-1 rounded-lg text-xs font-semibold shadow-sm flex items-center gap-1.5">
            ...
            <div className="p-5">
              ...
              <div className="flex flex-wrap gap-1.5 mb-5">
                {room.amenities.slice(0, 4).map((a) => (
                  <span key={a} className="text-[11px] text-foreground-muted bg-bg-cream px-2.5 py-1.5 rounded-lg border border-border leading-none">{a}</span>
                ))}
              ...
              <div className="flex items-center justify-between pt-4 border-t border-border">
                ...
                <div className="flex gap-1.5">
                  <button
                    ...
                    className="w-9 h-9 rounded-xl bg-bg-cream text-foreground-muted hover:text-primary hover:bg-primary/5 border border-border transition-all duration-200 hover:scale-105 active:scale-95 flex items-center justify-center"
                    title="Edit"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" style={{ animation: "fade-in 0.2s ease-out" }} />
          <div
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-bg-cream rounded-xl border border-border focus:border-primary focus:ring-2 focus:ring-primary/15 text-foreground placeholder-foreground-light/60 transition-all duration-200 outline-none"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-bg-cream rounded-xl border border-border focus:border-primary focus:ring-2 focus:ring-primary/15 text-foreground placeholder-foreground-light/60 transition-all duration-200 min-h-[80px] outline-none resize-none"
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">URL Gambar Utama</label>
                <input
                  type="url"
                  value={form.image}
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">URL Gambar Lainnya (pisahkan dengan koma)</label>
                <input
                  type="text"
                  value={form.gallery}
                ...
                />
                <p className="text-[11px] text-foreground-muted mt-1.5">Tambahkan URL gambar tambahan untuk galeri slideshow di halaman utama</p>
              </div>
              ...
              <div className="flex gap-3 pt-3">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="flex-1 py-3 border-2 border-border rounded-xl font-medium hover:bg-bg-cream transition-all duration-200 text-foreground-muted hover:text-foreground active:scale-[0.98]"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 rounded-xl font-medium bg-gradient-to-r from-primary to-primary-dark text-white shadow-lg shadow-primary/20 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 active:scale-[0.98]"
                >
